...
class TreePrinter:
    ...
    def print_representation(self):
        printing.print_line()
        log.info("Tree representation of the mapping engine:")
    ...
    def _print_stream(self, stream_handler, prefix="", last_stream=False):
        ...
        log.info(f"{prefix}{self.CORNER if last_stream else self.TEE}{self.HORIZONTAL} {stream}")
    ...
    def _print_trigger(self, trigger, uuid, prefix, last_trigger):
        log.info(f"{prefix}{self.TEE}{self.HORIZONTAL} {trigger}")
        action = self.mapping_engine._actions[uuid]
        log.info(f"{prefix}{' ' * 5}-> {action}")

    def _print_processor(self, processor, prefix, last_processor):
        log.info(f"{prefix}{self.CORNER if last_processor else self.TEE}{self.HORIZONTAL} {processor}")
